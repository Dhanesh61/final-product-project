...
<body>
    <nav class="navbar navbar-expand-sm bg-dark">
        <ul class="navbar-nav">
            <li class="nav-item">
                <a class="nav-link text-light" href="{{ route('website') }}">Update Website</a>
            </li>
        </ul>
    </nav>
        ...
        <div class="row justify-content-center"> 
            <div class="col-sm-8">
                <div class="card mt-3 p-3">
                    <form id="websiteForm" action="{{ route('website.update', $website->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="form-group">
                            <label for="websiteurl">Website URL:</label>
                            <input type="text" name="websiteurl" class="form-control" value="{{ old('websiteurl', $website->websiteurl) }}">
                            @error('websiteurl')
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" name="da" class="form-control" min="1" max="100" value="{{ old('da', $website->da) }}">
                            @error('da')
                                <div class="vali-red">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="form-group">
                            ...
                            <div>
                                @php
                                    ...
                                    $selectedCategories = explode(',', $website->category);
                                @endphp
                                @foreach($categories as $value => $label)
                                    <input type="checkbox" name="category[]" value="{{ $value }}" id="category{{ ucfirst($value) }}" {{ in_array($value, $selectedCategories) ? 'checked' : '' }}> 
                        ...
                        <div class="form-group">
                            ...
                            <div>
                                @php
                                    ...
                                    $selectedFcCategories = explode(',', $website->fcCategory);
                                @endphp
                                @foreach($fcCategories as $value => $label)
                                    <input type="checkbox" name="fcCategory[]" value="{{ $value }}" id="fcCategory{{ ucfirst($value) }}" {{ in_array($value, $selectedFcCategories) ? 'checked' : '' }}> 
                                    <label for="fcCategory{{ ucfirst($value) }}">{{ $label }}</label>
                                @endforeach
                                @error('fcCategory')
                                <div class="vali-red">{{ $message }}</div>
                                @enderror
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" name="categoryPrice" class="form-control" value="{{ old('categoryPrice', $website->categoryPrice) }}">
                            @error('categoryPrice')
                                <div class="vali-red">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="form-group">
                            ...
                            <input type="number" name="fcCategoryprice" class="form-control" value="{{ old('fcCategoryprice', $website->fcCategoryprice) }}">
